...
Mnemo should feel like a private memory layer, not a generic AI chat app. This pass chooses a calm **Mnemonic Thread** direction and implements the safest parts in the app: a reusable in-app mark, an ink-and-sage semantic palette, and updated source/privacy surfaces.
...
| App icon direction | Rounded ink tile with a white mnemonic-thread mark; optional sage variant for internal marks. | Works at small size, survives iOS icon masking, and avoids tiny details. | Added `MnemoApp/Design/MnemoLogoMark.svg` and `AppIconNotes-2026-07-07.md`. AppIcon asset set was not updated in this pass. |
| Colour palette | Ink and Sage. Primary ink `#172334`; accent sage `#2F6F5B`; pressed sage `#245747`; soft sage surfaces through semantic opacity. | Distinct from generic purple/blue AI palettes while still feeling secure, calm, and premium. | Updated `DS.Colours` semantic tokens and preserved existing aliases for low-risk adoption. |
...
| Privacy/security visual language | Identity mark plus calm ink/sage, not vaults, cyber styling, or fear-based security language. | Mnemo should feel protected by the device and user control, not paranoid. | App Lock uses the Mnemo mark and copy remains LocalAuthentication/no-account accurate. |
...
| --- | --- | --- |
| `primary` | Ink `#172334` | Brand mark fill, calm title emphasis. |
| `accent` | Sage `#2F6F5B` | Primary actions, tabs, source affordances. |
| `accentPressed` | Deep sage `#245747` | Pressed/active brand state. |
| `accentSoft` | Sage at low opacity | Private badges, icon wells, quiet emphasis. |
...
| `sourceCardBorder` | Sage border | Trust affordance without heavy decoration. |
| `appLockSurface` | Semantic surface | Lock screen card. |
| `privateBadgeSurface` | Soft sage | Private/local badges and logo variants. |
